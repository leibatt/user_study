...
def generate_field_for_question(question):
    if question.input_type in ["select","multiple"]: # get selections
        if question.parent_question is not None:
            ...
            responses = db_session.query(models.SurveyResponse).filter_by(question_id=question.parent_question).order_by(models.SurveyResponse.id).all()
        else:
            responses = db_session.query(models.SurveyResponse).filter_by(question_id=question.id).order_by(models.SurveyResponse.id).all()
